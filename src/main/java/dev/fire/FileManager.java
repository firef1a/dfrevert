...
public class FileManager {
    ...
    public static Path Path() {
        Path path = DFrevert.MC.runDirectory.toPath().resolve(DFrevert.MOD_ID);
        path.toFile().mkdir();
        return path;
    }

    public static Path templatesPath() {
        Path path = DFrevert.MC.runDirectory.toPath().resolve(DFrevert.MOD_ID).resolve("templates");
        path.toFile().mkdir();
        return path;
    }
    ...
    public static File getConfigFile() {
        return new File(FabricLoader.getInstance().getConfigDir().toFile(), DFrevert.MOD_ID + ".json");
    }
